	...
	: dep(dep_)
{
	...
	if (auto inner_plain_dep= to <const Plain_Dep> (inner_dep)) {
		Hash_Dep hash_dep_base(inner_plain_dep->target.flags,
			inner_plain_dep->target.name.unparametrized());
	...
	shared_ptr <const Dep> dep_source)
{
	...
	TRACE("dep_result= %s; flags= %s; dep_source= %s",
		show_trace(dep_result), show(Flags_View(flags)), show_trace(dep_source));
