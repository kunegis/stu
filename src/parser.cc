	...
	std::vector <shared_ptr <const Plain_Dep> > targets)
{
	...
	while (is <Flag_Token> ()) {
		...
		if (flag_token->flag_char == flag_chars[I_PERSISTENT]) {
			...
			flag_token->get_place() << fmt("flag %s cannot be used",
				show(flag_token));
	...
	if (! is <Name_Token> ()) {
		if (iter == tokens.end()) {
			...
			(*iter)->get_place_start() << fmt(
				"expected a filename, a flag, or %s, not %s",
	...
	for (size_t jj= 0; jj < name_copy_src->get_n(); ++jj) {
		...
		if (parameters.count(parameter) == 0) {
			name_copy_src->places[jj] << fmt(
				"parameter %s cannot appear in copied file %s",
	...
	shared_ptr <const Plain_Dep> &target_first)
{
	...
	if (! target_name->find_duplicate_parameters(param_1, param_2)) {
		place_of_target << fmt(
			"the two parameters %s and %s in the name %s must be separated by at least one character",
			show(Prefix_View("$", param_1)),
	...
	if (! place_output_new.empty() && flags.get_flags() & F_NO_FOLLOW) {
		place_output_new << fmt("output redirection using %s cannot be used",
			show(Operator_View('>')));
		flags.place_by_index(I_NO_FOLLOW) << fmt(
	...
	const std::vector <shared_ptr <const Plain_Dep> > &targets)
{
	...
	if (parse_expression_list(r, name_input, place_input, targets)) {
		assert(r.size() >= 1);
		if (r.size() > 1) {
	...
	if (next_concatenates()) {
		...
		bool rr= parse_expression(next, name_input, place_input, targets);
	...
	const std::vector <shared_ptr <const Plain_Dep> > &targets)
{
	...
	parse_expression_list(content, name_input, place_input, targets);
	...
	for (auto &j: content) {
		if (j->flags.get_flags() & F_VARIABLE) {
			j->get_place() << fmt("variable dependency %s cannot appear",
				show(j));
	...
	const std::vector <shared_ptr <const Plain_Dep> > &targets)
{
	...
	/* Flags */
	Placed_Flags flags;
	flags.add_unplaced_index(I_VARIABLE);
		...
		is_flag(flag_chars[I_TRIVIAL]))
	{
		...
		if (is_flag(flag_chars[I_PERSISTENT])) {
			flags.add_placed_index(I_PERSISTENT,
				place_flag_last);
			...
			if (! option_a) {
				flags.add_placed_index(I_TRIVIAL, place_flag_last);
			}
		} else {
			unreachable();
		}
	...
	if (is_operator('<')) {
		...
		flags.add_unplaced_index(I_INPUT);
		++iter;
	}
	...
	if (! is <Name_Token> ()) {
		if (iter == tokens.end())
			...
			(*iter)->get_place_start() <<
				fmt("expected a filename, not %s", show(*iter));
		if (has_input) {
			place_input << fmt("after %s", show(Operator_View('<')));
	...
	if (has_input && ! name_input.empty()) {
		...
		name_input.place << fmt(
			"shadowing previous input redirection %s",
			show(Prefix_View("<", name_input)));
		if (targets.size() == 1) {
			targets.front()->place <<
